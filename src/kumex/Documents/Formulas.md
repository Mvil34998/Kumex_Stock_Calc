...
> Если размер плиты или количество отличается, считать их входными параметрами:
> `plate_length`, `plate_width`, `plate_thickness`, `plate_count`.
...
Уточнения:
- Если **нет ни одного значения ≤ 52**, позицию пометить как `issues` и не считать списание.
- Если несколько размеров равны `t` (например, два раза 52), любой из них допустим, т.к. на площадь это не влияет.

### 2.2 Пятно раскроя (footprint)
Две оставшиеся стороны (не равные `t`) образуют прямоугольник “пятна”:
- `p` и `q` — оставшиеся размеры (мм)

Уточнение: порядок `p`/`q` не важен; для детерминизма можно сортировать по убыванию.
...
2) Из оставшихся двух сторон выбрать **ширину полосы** `w` (это сторона, не являющаяся длиной профиля)
   - Если в данных нет явной длины, считать длиной **наибольшую** из двух оставшихся сторон, а `w` — меньшую.
3) Расход по м²:
   \[
...
> Единица по умолчанию: мм³. При необходимости переводить в см³ делением на 1000.
...
1) Если среди `(a,b,c)` **нет значения ≤ 52**, то деталь **не из плит 52 мм** → в `issues`, списание не считать.
2) Проверка раскроя по ширине листа 1000 мм:
   - если суммарная ширина полос > 1000 без учёта раскладки, считать отдельным кейсом для “packing/nest” или коэффициента отхода.
