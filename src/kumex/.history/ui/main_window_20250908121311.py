            ...
            for i, line in enumerate(lines):
                ...
                try:
                    ...
                    for iid in tree.get_children():
                        tree.delete(iid)
                    # вставить новые
                    for rec in self.material_rows:
                        tree.insert(
                            "", "end",
                            values=(rec.get("desc",""), rec.get("qty",""), rec.get("po",""), rec.get("date",""))
                        )
                    # счётчик позиций
                    self.mat_count_lbl.config(text=f"Positsioone: {len(self.material_rows)}")
                except Exception as e:
                    print(f"[WARN] unable to repaint material table: {e}")
