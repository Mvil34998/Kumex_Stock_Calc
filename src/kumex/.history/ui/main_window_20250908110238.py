...
class MainWindow(tk.Frame):
    ...
    def _parse_materials(self):
        """
        Адаптивный парсинг таблицы материалов из PDF:
        - строки формируются по кластерам по оси Y (без жёстких констант);
        - раскладка по колонкам — по диапазонам X;
        - работает на нескольких страницах.
        Результат складывается в self.material_rows (list of dict).
        """
        from pathlib import Path
        import pdfplumber

        # Диапазоны X для колонок (под твой макет GRN)
        DESC_X = (50, 345)     # "Material / Size"
        QTY_X  = (360, 405)    # "Qty" (Received)
        PO_X   = (420, 475)    # "PO No."
        DATE_X = (495, 565)    # "Date"

        # Грубая фильтрация по Y, чтобы отсечь колонтитулы; оставим широкое окно
        SAFE_Y_MIN = 80
        SAFE_Y_MAX = 820

        # Порог «слипания» слов в одну строку (насколько близки по Y)
        ROW_MERGE_THR = 6.0    # пунктов

        self.material_rows = []

        for pdf_path in getattr(self, "pdf_files", []):
            pdf_name = Path(pdf_path).name if not hasattr(pdf_path, "name") else pdf_path.name
            try:
                with pdfplumber.open(str(pdf_path)) as pdf:
                    for page in pdf.pages:
                        ...
                        # 1) Группируем слова в строки по близости по Y (адаптивно)
                        lines = []  # список: [(y_ref, [words])]
                        for w in words:
                            top = float(w["top"])
                            bottom = float(w["bottom"])
                            if bottom < SAFE_Y_MIN or top > SAFE_Y_MAX:
                                continue

                            y_mid = (top + bottom) / 2.0
                            placed = False
                            for i, (y_ref, lst) in enumerate(lines):
                                if abs(y_mid - y_ref) <= ROW_MERGE_THR:
                                    # добавляем в существующую строку, пересчитаем опорный y
                                    new_y = (y_ref * len(lst) + y_mid) / (len(lst) + 1)
                                    lines[i] = (new_y, lst + [w])
                                    placed = True
                                    break
                            if not placed:
                                lines.append((y_mid, [w]))

                        # 2) Отсортировать строки сверху вниз
                        lines.sort(key=lambda t: t[0])

                        # 3) Собрать поля по колонкам X
                        for _, lst in lines:
                            # раскладываем слова по колонкам
                            desc_parts, qty_parts, po_parts, date_parts = [], [], [], []
                            for w in sorted(lst, key=lambda x: float(x["x0"])):
                                xmid = (float(w["x0"]) + float(w["x1"])) / 2.0
                                t = w["text"].strip()

                                if DESC_X[0] <= xmid <= DESC_X[1]:
                                    desc_parts.append(t)
                                elif QTY_X[0] <= xmid <= QTY_X[1]:
                                    qty_parts.append(t)
                                elif PO_X[0] <= xmid <= PO_X[1]:
                                    po_parts.append(t)
                                elif DATE_X[0] <= xmid <= DATE_X[1]:
                                    date_parts.append(t)

                            desc = " ".join(desc_parts).strip()
                            qty  = "".join(qty_parts).strip()
                            po   = "".join(po_parts).strip()
                            ...
                            # нормализуем qty (оставим как текст; приводим запятую к точке)
                            qty = qty.replace(",", ".")
                ...
                print(f"[WARN] PDF parse failed: {pdf_name} -> {e}")
    ...
    def _calc_m2(self):
        ...
        for row in self.material_rows:
            ...
            # Количество может прийти «замусоренным» (например, из заголовка "Qty / Number").
            # Достаём из текста первую группу цифр, иначе считаем 0.
