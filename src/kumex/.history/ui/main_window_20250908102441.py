...
class MainWindow(tk.Frame):
    ...
    def _parse_materials(self):
        """Парсинг PDF по фиксированным координатам (жёсткая сетка)."""
        ...
        if not self.pdf_files:
            self._set_status("PDF puuduvad.")
            self.mat_count_lbl.config(text="Positsioone: 0")
            return

        # Координаты таблицы (подогнаны по pdf_words_dump.txt)
        TABLE_Y_TOP    = 155
        TABLE_Y_BOTTOM = 790
        ROW_H          = 18
            ...
            try:
                with pdfplumber.open(str(p)) as pdf:
                    for page in pdf.pages:
                        y = TABLE_Y_TOP
                        while y + ROW_H <= TABLE_Y_BOTTOM:
                            y0, y1 = y, y + ROW_H

                            def read_cell(x0, x1):
                                area = (x0, y0, x1, y1)
                                txt = page.within_bbox(area).extract_text(
                                    x_tolerance=1, y_tolerance=1
                                )
                                return (txt or "").strip()

                            desc = read_cell(*DESC_X)
                            qty_s = read_cell(*QTY_X)
                            po  = read_cell(*PO_X) or "?"
                            od  = read_cell(*DATE_X) or "?"

                            if not (desc or qty_s or po or od):
                                y += ROW_H
                                continue
                            ...
                            if desc and qty_val is not None:
                                ...
                                if "esd" in dlow:
                                    mat_name = ""
                                elif "pom" in dlow and "valge" in dlow:
                                    mat_name = "POM Valge"
                                elif "pom" in dlow and ("must" in dlow or "õhuke" in dlow):
                                    mat_name = "POM Must"
                                else:
                                    mat_name = ""
                                ...
                                self.mat_tree.insert("", "end", values=(desc, qty_val, po, od))
                                total += 1

                            y += ROW_H
            except Exception as e:
                self._set_status(f"Viga {p.name}: {e}")
