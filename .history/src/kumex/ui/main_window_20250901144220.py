...
class MainWindow(tk.Frame):
    ...
    def _build_ui(self):
        ...
        ttk.Label(container, text="Date :").grid(row=0, column=0, padx=8, pady=8, sticky="w")

        year_cb = ttk.Combobox(container, textvariable=self.year_var, values=years, width=6, state="readonly")
        year_cb.grid(row=0, column=1, padx=(0, 4), pady=8, sticky="w")

        month_cb = ttk.Combobox(container, textvariable=self.month_num_var, values=months, width=4, state="readonly")
        month_cb.grid(row=0, column=2, padx=(0, 8), pady=8, sticky="w")
