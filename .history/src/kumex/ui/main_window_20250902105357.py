...
class MainWindow(tk.Frame):
    ...
    def _open_stock_dialog(self):
        ...
        ttk.Label(op_box, text="Тип:").grid(row=0, column=5, padx=8, pady=6, sticky="w")
        ...
        ttk.Button(topbar, text="Отменить выбранную", command=self._undo_selected).pack(side="right", padx=(4, 0))
    ...
    def _reload_ledger(self):
        ...
        for rec in ledger:
            ...
            if hasattr(self, "_ledger_tree"):
                ...
                # уникальный iid (по ts + порядковому номеру), чтобы потом найти запись
                iid = f"{rec.get('ts','')}-{cnt}"

                # запоминаем, что лежит под этой строкой (для undo)
                from decimal import Decimal
                try:
                    amt_decimal = Decimal(rec.get("amount_m2", 0))
                except Exception:
                ...
                self._ledger_index[iid] = {
                    ...
                    "month": month,
                    "material": material,
                    "type": raw_type,            # manual_add/manual_sub/month_calc/...
                    "amount": amt_decimal,
                    "note": note,
                }

                # вставляем строку в таблицу
                self._ledger_tree.insert("", "end", iid=iid,
