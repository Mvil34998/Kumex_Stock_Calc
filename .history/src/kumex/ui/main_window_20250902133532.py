...
class MainWindow(tk.Frame):
    ...
    def _open_stock_dialog(self):
        ...
        ttk.Label(op_box, text="Тип действия:").grid(row=0, column=5, padx=8, pady=6, sticky="w")
        ...
        self._stock_win.update_idletasks()  # чтобы размеры посчитались
        ...
        # фактический размер диалога
        ww = self._stock_win.winfo_width()
        wh = self._stock_win.winfo_height()
        if ww <= 1 or wh <= 1:  # на всякий случай, если ещё 1x1
            ww = self._stock_win.winfo_reqwidth()
            wh = self._stock_win.winfo_reqheight()

        # координаты так, чтобы окно было по центру главного
        x = px + (pw - ww) // 2
        y = py + (ph - wh) // 2

        self._stock_win.geometry(f"+{x}+{y}")
        self._stock_win.transient(self.master)  # поверх главного и в его таскбар не выносится
        self._stock_win.focus_set()
        # --- конец центрирования ---
