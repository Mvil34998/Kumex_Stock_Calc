...
class MainWindow(tk.Frame):
    ...
    def _parse_materials(self):
        ...
        def _parse_materials(self):
            """
            Парсит материалы из PDF-файлов, используя табличную структуру.
            """
        # очистка списка перед циклом
        for iid in self.mat_tree.get_children():
            self.mat_tree.delete(iid)
        self.material_rows = []
        ...
        for p in self.pdf_files:
            try:
                with pdfplumber.open(p) as pdf:
                    # Найдём PO Number и дату в первой таблице документа
                    po = "N/A"
                    od = "N/A"
                    if pdf.pages:
                        main_table = pdf.pages[0].extract_tables()[0]
                        for row in main_table:
                            if row[0] == "PO Number":
                                po = row[1].strip()
                            elif row[0] == "Received Date":
                                od = row[1].strip()

                    # Теперь парсим таблицу с материалами
                    for page in pdf.pages:
                        tables = page.extract_tables()
                        for table in tables:
                            # Проверяем, что это нужная нам таблица с товарами
                            if not table:
                                continue
                            
                            header = table[0]
                            if "Description" in header and "PartNo" in header:
                                ...
                                for row in table[1:]:
                                    ...
                                    # Пытаемся извлечь количество (qty)
                                    try:
                                        qty = int(qty_str)
                                    ...
                                    # Определяем материал на основе описания
                                    d = desc.lower()
                                    material_name = ""
                                    if "pom" in d and "valge" in d:
                                        material_name = "POM Valge"
                                    elif "pom" in d and ("must" in d or "õhuke" in d):
                                        # Также игнорируем ESD-материалы, как в вашей предыдущей логике
                                        if "esd" not in d:
                                            material_name = "POM Must"
                                    
                                    # Если материал найден, добавляем запись
                                    if material_name:
                ...
                self._set_status(f"Viga faili töötlemisel {Path(p).name}: {e}")
                continue
