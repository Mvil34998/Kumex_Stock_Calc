...
class MainWindow(tk.Frame):
    ...
    def _parse_materials(self):
        ...
        qty_line_rx = re.compile(r"^\s*\d+\s+\d+\s+(\d+)\b")  # пример: "1 70056 2000 ..."
        ...
        for p in self.pdf_files:
            ...
            for i, line in enumerate(lines):
                ...
                # по образцам Kumex qty находится В ПРЕДЫДУЩЕЙ строке
                qty = None
                if i - 1 >= 0:
                    m = qty_line_rx.match(lines[i - 1])
                    if m:
                        try:
                            qty = int(m.group(1))
                        except Exception:
                ...
                # если qty не нашли, попробуем ещё на 2 строки вверх (редкие случаи)
                if qty is None and i - 2 >= 0:
                    m = qty_line_rx.match(lines[i - 2])
                    if m:
                        try:
                            qty = int(m.group(1))
                        except Exception:
