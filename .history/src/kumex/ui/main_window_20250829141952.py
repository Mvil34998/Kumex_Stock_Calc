...
class MainWindow(tk.Frame):
    ...
    def _build_ui(self):
        ...
        report_btn = ttk.Button(container, text="Сформировать отчёт", command=self._parse_materials)
    ...
    def _parse_materials(self):
        ...
        # Очистка ПЕРЕД циклом (важно не очищать внутри)
        self.mat_list.delete(0, tk.END)
        self.material_rows = []
        ...
        # Паттерны под твои Kumex-файлы (при необходимости расширим)
        size_rx = re.compile(r"\b\d+\s*[*xX]\s*\d+(?:\s*[*xX]\s*\d+)?\s*mm\b")
        qty_line_rx = re.compile(r"^\s*\d+\s+\d+\s+(\d+)\b")  # пример строки с qty: "1 70056 2000 ..."
        po_rx = re.compile(r"\bPO\s*Number\s*([A-Za-z0-9_-]+)")
        date_rx = re.compile(r"\bOrder\s*Date\s*([0-9]{1,2}\.[0-9]{1,2}\.[0-9]{4})")
        ...
        for p in self.pdf_files:
            ...
            for i, line in enumerate(lines):
                # Ищем строку-описание, в которой встречаются размеры
                if not size_rx.search(line):
                    continue

                # По твоим образцам qty — в ПРЕДЫДУЩЕЙ строке (иногда на 2 строки выше)
                qty = None
                for back in (1, 2):
                    if i - back >= 0:
                        m = qty_line_rx.match(lines[i - back])
                        if m:
                            try:
                                qty = int(m.group(1))
                            except Exception:
                ...
                desc = line
                item_str = f"{desc} | qty={qty} (PO {po}, {od})"

                # добавляем в общий список (НЕ очищаем внутри цикла по файлам)
                self.material_rows.append(item_str)
                self.mat_list.insert(tk.END, item_str)
                total += 1

        # Обновляем счётчики и статус уже после обработки ВСЕХ файлов
