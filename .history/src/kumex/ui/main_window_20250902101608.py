...
class MainWindow(tk.Frame):
    ...
    def _build_ui(self):
        ...
        btn_row.grid(row=_row, column=0, columnspan=4, sticky="ew", padx=8, pady=(8, 6))
        ...
        ttk.Entry(
            conv_group, textvariable=self.kerf_mm_var, width=6
        ).grid(
    ...
    def _open_stock_dialog(self):
        ...
        ttk.Label(op_box, text="Количество, м²:").grid(row=0, column=2, padx=8, pady=6, sticky="w")
        ttk.Entry(op_box, textvariable=self._op_amount, width=10).grid(row=0, column=3, padx=(0, 8), pady=6, sticky="w")

        ttk.Label(op_box, text="Тип:").grid(row=0, column=4, padx=8, pady=6, sticky="w")
        ttk.Radiobutton(op_box, text="+ Пополнение", variable=self._op_type, value="add").grid(row=0, column=5, padx=(0, 8), pady=6, sticky="w")
        ttk.Radiobutton(op_box, text="– Списание",   variable=self._op_type, value="sub").grid(row=0, column=6, padx=(0, 8), pady=6, sticky="w")
        ...
        cols = ("ts", "month", "material", "type", "amount", "note")
        ...
        self._ledger_tree.heading("type", text="Тип")
        ...
        self._ledger_tree.column("type", width=110, anchor="w")
    ...
    def _reload_ledger(self):
        ...
        if hasattr(self, "_ledger_tree"):
            for iid in self._ledger_tree.get_children():
                self._ledger_tree.delete(iid)
        ...
        for rec in ledger:
            ...
            if hasattr(self, "_ledger_tree"):
                self._ledger_tree.insert("", "end", values=(ts_disp, month, material, typ, amt, note))
                cnt += 1

        if hasattr(self, "_stock_status"):
            self._stock_status.set(f"Записей в журнале: {cnt}")
