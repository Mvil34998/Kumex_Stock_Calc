...
class MainWindow(tk.Frame):
    def __init__(self, master=None):
        ...
        self.materials_cfg = {
            "POM valge": {"enabled": tk.BooleanVar(value=True),
                        "stock_m3": tk.StringVar(value="0.00"),
                        "remain_m3": tk.StringVar(value="0.00")},
            "POM must":  {"enabled": tk.BooleanVar(value=True),
                        "stock_m3": tk.StringVar(value="0.00"),
                        "remain_m3": tk.StringVar(value="0.00")},
        }
        ...
        # kumex_stock.json: путь и загрузка материалов
        self.stock_path = self.state_dir / "kumex_stock.json"
        stock_data = load_json(self.stock_path, default={"materials": {}})

        # Построим конфиг материалов из JSON (enabled/stock_m3/remain_m3), БЕЗ математики
        self.materials_cfg = {}
        for name, rec in stock_data.get("materials", {}).items():
            self.materials_cfg[name] = {
                "enabled": tk.BooleanVar(value=bool(rec.get("enabled", True))),
                "stock_m3": tk.StringVar(value=f'{float(rec.get("stock_m3", 0)):0.2f}'),
                "remain_m3": tk.StringVar(value=f'{float(rec.get("remain_m3", 0)):0.2f}')
            }
    ...
    def _build_ui(self):
        ...
        self.mat_tree.column("desc", width=200, anchor="w")
        ...
        for name, cfg in self.materials_cfg.items():
            ...
            ttk.Checkbutton(cfg_group, variable=cfg["enabled"]).grid(row=_row, column=1, padx=8, pady=2, sticky="w")
        ...
        # Ряды по материалам из JSON
        _row = 1
        for name, cfg in self.materials_cfg.items():
            ttk.Label(cfg_group, text=name).grid(row=_row, column=0, padx=8, pady=2, sticky="w")

            # галочка "Учитывать" — только включение/выключение + сохранение в JSON
            ttk.Checkbutton(
                cfg_group,
            ...
            # поля "Склад, м³" и "Остаток, м³" — пока просто отображение/ввод (без математики)
            e_stock = ttk.Entry(cfg_group, textvariable=cfg["stock_m3"], width=10)
            e_stock.grid(row=_row, column=2, padx=8, pady=2, sticky="w")
    ...
    def _save_stock_enabled(self, material_name: str):
        ...
        if material_name not in mats:
            mats[material_name] = {
                ...
                "stock_m3": float(self.materials_cfg[material_name]["stock_m3"].get() or 0),
                "remain_m3": float(self.materials_cfg[material_name]["remain_m3"].get() or 0),
            }
        else:
