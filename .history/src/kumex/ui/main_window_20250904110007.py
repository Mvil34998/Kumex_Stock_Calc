...
class MainWindow(tk.Frame):
    ...
    def _build_ui(self):
        ...
        cfg_group = ttk.LabelFrame(bottom_frame, text="Kumex ladu — материалы для учёта (m²)")
...
# --- конец новой строки ---
    ...
    def _add_stock_operation(self):
        ...
        if mat not in ("POM Valge", "POM Must"):
            messagebox.showerror("Viga", "Выберите материал.")
            return
    ...
    def _undo_selected(self):
        ...
        if not sel:
            messagebox.showwarning("Отмена", "Valige kirje logis.")
            return

        iid = sel[0]
        meta = getattr(self, "_ledger_index", {}).get(iid)
        if not meta:
            messagebox.showerror("Viga", "Не удалось найти данные по выбранной записи.")
            return
        ...
        if amount <= 0:
            messagebox.showerror("Viga", "Количество в записи равно 0.")
            return
    ...
    def _apply_stub(self):
        ...
        if mkey in set(data.get("closed_months", [])):
            messagebox.showinfo("Arvestus on juba tehtud", f"Месяц {mkey} закрыт. Расчёт был произведён ранее.")
            self._update_calc_button_state()
            return
        ...
        if valge == 0 and must == 0:
            messagebox.showwarning("Нет данных", "Valitud kuu kohta ei ole materjale mahakandmiseks.")
            return
