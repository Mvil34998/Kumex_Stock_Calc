...
class MainWindow(tk.Frame):
    ...
    def _build_ui(self):
        ...
        # Строки итогов (только вывод) — динамически для всех материалов из JSON
        _row_conv = 0
        for name in self.conv_totals.keys():
            ttk.Label(conv_group, text=f"{name}:").grid(
                row=_row_conv, column=0, padx=8,
                pady=(6 if _row_conv == 0 else 2, 2), sticky="w"
            )
            ttk.Entry(
                conv_group, textvariable=self.conv_totals[name],
                width=12, state="readonly"
            ).grid(
                row=_row_conv, column=1, padx=8,
                pady=(6 if _row_conv == 0 else 2, 2), sticky="w"
            )
            _row_conv += 1
